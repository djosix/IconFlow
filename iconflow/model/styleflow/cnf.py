...
class CNF(nn.Module):
    def __init__(self, odefunc, conditional=True, T=1.0, train_T=False, regularization_fns=None,
        ...
        self.T = T
        if train_T:
            self.register_parameter("sqrt_end_time", nn.Parameter(torch.sqrt(torch.FloatTensor([T])[0])))
        ...
        if integration_times is None:
            if self.train_T:
               ...
               # print("integration times:", integration_times)
                ...
                integration_times = torch.FloatTensor([0., self.T], requires_grad=False).to(x)
